...
def main():
    with tab1:
        ...
        if st.button("Classify!") and image is not None:
            ...
            res = requests.post(
                "http://158.160.168.132:8000/clf_image", files=files
            ).json()
            # print results
            st.write(
    ...
    with tab2:
        ...
        if st.button("Classify"):
            text = {"text": txt}
            res = requests.post("http://158.160.168.132:8000/clf_text", json=text)
    ...
    with tab3:
        ...
        with st.form("query_form"):
            ...
            if submitted and feature1 and feature2:
                ...
                # send data and get the result
                res = requests.post(
                    "http://158.160.168.132:8000/clf_table", json=vector
                )
